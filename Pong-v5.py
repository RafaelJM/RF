...
class ReplayBuffer:
    def __init__(self, max_size, input_shape):
        ...
        self.buffer = np.zeros(max_size, dtype=[
            ('observations', np.uint8, input_shape),
            ('actions', np.uint8),
            ('rewards', np.int32),
            ('future_returns', np.int32),
    ...
    def add(self, observation, action, reward):
        self.buffer[self.index] = (observation, action, reward, 0, 1.0) # Inicializa a prioridade como 1.0 (ou outra constante)
...
# Definindo a arquitetura do modelo para o jogo Breakout
...
plt.imshow(preprocess_state(state), cmap='gray')  # Exibe em grayscale
plt.title("Visão do computador")
plt.savefig(f'./{game}/comp_vision.png', dpi=300)
plt.show()
...
replay_buffer = ReplayBuffer(1000000, (84,84,1))
